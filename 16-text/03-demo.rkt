#lang racket/base
;; =========================================================
;; 16-text/03-demo.rkt —— 第三步：综合，3D 场景 + 屏幕文字
;; 运行：racket 16-text/03-demo.rkt     T = 文字开/关   点X = 退出
;; =========================================================
;; 本课前两步：烤字帖画文字(01)、中文(02)。本步**不引入新语法**，把老教程
;; 15-text 的成品拼出来：3D 场景 + 屏幕文字 + FPS + 3D 标签投影。
;; 文字是"第二遍叠加"（先画 3D，再叠一层文字），和 13 课 FBO 两遍同思路。
;; =========================================================
...
(define-values (frame canvas)
  (make-window #:title "16-03 3D + 文字（综合）"
               #:width 800 #:height 600 #:draw draw #:on-char on-char))

(define prog-scene (send canvas with-gl-context (lambda () (build-program cube-vert cube-frag))))
