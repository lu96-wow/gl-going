#lang racket/base
;; =========================================================
;; 05-animate/03-branch.rkt —— 第三步：分情况画（when/unless/cond）
;; 运行：racket glsl/05-animate/03-branch.rkt    点 X = 退出
;; =========================================================
...
;;         [条件2 语句...]              else if (条件2) { ... }
;;         [else 语句...])              else { ... }
;;   条件里的 (and a b) (or a b) (not a) 是 04 课学过的逻辑运算符。
;;   ★注意：05 课 if 是"语句"（挑一段代码执行），04 课的三元 ?: 是"表达式"
;;   （挑一个值）。两者名字像，用途不同。
;;
...
(define-values (frame canvas)
  (make-window #:title "05-03 右上角色板（分支）" #:width 400 #:height 400 #:draw draw))
