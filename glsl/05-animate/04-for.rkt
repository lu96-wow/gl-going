#lang racket/base
;; =========================================================
;; 05-animate/04-for.rkt —— 第四步：for 循环画条纹
;; 运行：racket glsl/05-animate/04-for.rkt    点 X = 退出
;; =========================================================
...
;;   注意 DSL 的 (set! s (+ s x)) 就是 GLSL 的 s = s + x（也有 s += x 简写）。
(define frag-src
  (glsl (version 330 core)
        (in vec2 vUV)
        (uniform float uTime)
...
(define-values (frame canvas)
  (make-window #:title "05-04 for 条纹" #:width 400 #:height 400 #:draw draw))
