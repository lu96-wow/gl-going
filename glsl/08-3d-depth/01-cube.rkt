#lang racket/base
;; =========================================================
;; 08-3d-depth/01-cube.rkt —— 第一步：3D 顶点 + 立方体（线框）
;; 运行：racket glsl/08-3d-depth/01-cube.rkt    点 X = 退出
;; =========================================================
;; 07 课所有顶点都是 2D 的 (vec2 aPos)。本步升到 3D，画第一个 3D 物体：立方体。
;;
...
;;   ★注意顶点着色器里的 (vec4 aPos 1.0)：3D 位置补 1.0（不是 07 课的
;;   0.0 1.0 两段），因为平移要作用在 w=1 上，z 也要跟着矩阵走。
;;
;; ★"假相机"：顶点要落在 -z 方向才能被看见，所以先把世界往后推 6 个单位
;;   （V = 平移 (0,0,-6)），立方体仍放在原点。真正的相机 09 课讲 lookAt。
;;   本步用正交投影（07 课的 mat4-ortho，只是给 z 也设了范围）——透视下一步。
;;
...
(define-values (frame canvas)
  (make-window #:title "08-01 3D 立方体（线框）" #:width 400 #:height 400 #:draw draw))
