...
07 课在 2D 像素世界里摆弄矩阵。本课升到 3D，只多两个概念，却让"**近大远小**"
和"**前后遮挡**"同时成立：透视投影 + 深度缓冲。顶点位置从 vec2 变成 vec3，
顺便认识真正的 3D 物体——立方体。

本文件夹的 lib 采用"每课一份复制"；**本课的 lib.rkt 把矩阵升级到 3D**：

- `lib-gui.rkt` —— 从 07 复制（本课未改动；make-window 已申请 24 位深度缓冲，见下）
...
- `lib.rkt` —— 从 07 复制，升级：mat4-translate / mat4-scale 加 z 参数，新增 mat4-rot-x / mat4-rot-y；
