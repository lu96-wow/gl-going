# 09 相机 —— 视图矩阵 + 轨道相机 + 鼠标交互

按顺序运行，每步只在前一步基础上加 1–2 个新东西。

08 课相机是写死的"往后退 6 格"。本课解决"相机怎么动"。核心一句话：
**相机不是世界动了，而是"把整个世界搬到相机面前"**——视图矩阵 V 把世界坐标
换成以相机为原点的坐标。本课做轨道相机：鼠标拖动环绕、滚轮拉近拉远。

本文件夹的 lib 采用"每课一份复制"：

- `lib-gui.rkt` —— 从 08 复制，第 3 步扩展：make-window 加 `#:on-char` / `#:on-event`
- `lib.rkt` —— 从 08 复制，第 1 步裸写 mat4-look-at，第 4 步收进 lib（连同网格地面数据）

1. `01-lookat.rkt` **视图矩阵 mat4-look-at**：f/s/u 三个基向量，固定相机看原点 + 网格地面
2. `02-orbit.rkt`  **球坐标轨道相机**：yaw/pitch/dist → 眼睛位置，相机自动绕场景转
3. `03-input.rkt`  **鼠标交互**：左键拖 = 环绕，滚轮 = 拉近拉远（make-window 加 on-event/on-char）
4. `04-lib.rkt`    **收进 lib**：mat4-look-at + 网格地面（节奏步）
5. `05-demo.rkt`   **综合**（无新语法）：网格 + 中央立方体 + 散落立方体 + 完整轨道相机 + R 复位
