#lang racket/base
;; =========================================================
;; 09-camera/01-lookat.rkt —— 第一步：视图矩阵 mat4-look-at
;; 运行：racket glsl/09-camera/01-lookat.rkt    点 X = 退出
;; =========================================================
;; 08 课的相机是写死的"把世界往后推 6 格"。本步换真正的相机：视图矩阵。
;;
;; 本步新增（2 个）：
...
(define-values (frame canvas)
  (make-window #:title "09-01 视图矩阵" #:width 600 #:height 600 #:draw draw))
...
;; 立方体 VAO（复用 08 课的 cube-verts/cube-idx）
(define vao-cube
  (send canvas with-gl-context
        (lambda ()
          (define vbo (u32vector-ref (glGenBuffers 1) 0))
