#lang racket/base
;; =========================================================
;; 17-text/01-atlas.rkt —— 第一步：烤字帖 + 画一行文字
;; 运行：racket glsl/17-text/01-atlas.rkt    点 X = 退出
;; =========================================================
;; 前面 16 课 GPU 只画"几何"。屏幕上的字符从哪来？本步把文字也变成几何。
;;
;; 本步新增（2 组）：
...
;;     文字 = 用 alpha 雕刻字形的片（所以 12 课的混合在这里派上用场）。
;;
...
;; uProj 把像素坐标直接映射成 NDC（07 课的正交投影）。
(define text-vert
...
(define-values (frame canvas)
  (make-window #:title "17-01 烤字帖 + 画文字" #:width 800 #:height 400 #:draw draw))
