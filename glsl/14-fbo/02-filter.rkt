#lang racket/base
;; =========================================================
;; 14-fbo/02-filter.rkt —— 第二步：后处理滤镜
;; 运行：racket glsl/14-fbo/02-filter.rkt    1/2/3/4 = 原图/反色/灰度/暗角   点X = 退出
;; =========================================================
;; 上一步：离屏管线通了，但"原样采样"看不出名堂。本步让每个像素在贴回时
;; 被改写——这就是后处理滤镜。
;;
;; 本步新增（1 个）：
;;   滤镜 —— 在整屏贴回的片元着色器里，对每个像素颜色 c 做一次改写
;;
;; ★为什么这招厉害：屏幕每个像素 = 采样离屏纹理。于是"对画面做全局效果"
;;   变成了"对单个像素写一个函数"——反色、灰度、暗角都只是几行算术。
;;   uMode 切滤镜：
;;     反色  c = 1 - c
;;     灰度  g = dot(c, (0.299,0.587,0.114))（人眼亮度权重），c = (g,g,g)
;;     暗角  c *= 1 - 0.55×length(vUV-0.5)（越靠边越暗）
;; =========================================================
...
(define-values (frame canvas)
  (make-window #:title "14-02 后处理滤镜（1-4 切换）"
               #:width 600 #:height 600 #:draw draw #:on-char on-char))

(define prog-scene  (send canvas with-gl-context (lambda () (build-program (GL_VERTEX_SHADER scene-vert) (GL_FRAGMENT_SHADER scene-frag)))))
(define prog-screen (send canvas with-gl-context (lambda () (build-program (GL_VERTEX_SHADER screen-vert) (GL_FRAGMENT_SHADER screen-frag)))))
