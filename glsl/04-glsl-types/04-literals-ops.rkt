#lang racket/base
;; =========================================================
;; 04-glsl-types/04-literals-ops.rkt —— 第四步：字面量纪律 + 运算符
;; 运行：racket glsl/04-glsl-types/04-literals-ops.rkt    点 X = 退出
;; =========================================================
;; 上一步：会用 swizzle 重排分量。本步补上两个"写表达式"时最容易踩的坑。
;;
;; 本步新增（2 组，同属"写表达式"这一件事）：
;;   ① 字面量纪律 —— 浮点必须带小数点；★★ 整数除法 1/2 = 0，不是 0.5！★★
;;   ② 运算符 —— 算术(+ - * /)逐分量；比较(< > == !=)产出 bool；三元 ?: 选值
;;
...
;;   语句版的 if/for 等控制流留到 05 课——本课只用表达式。
;;
;; 本步视觉（一次把上面全展示）：
...
(define-values (frame canvas)
  (make-window #:title "04-04 字面量与运算符" #:width 400 #:height 400 #:draw draw))
