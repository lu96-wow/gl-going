#lang racket/base
;; =========================================================
;; 04-glsl-types/01-quad.rkt —— 第一步：铺满窗口的四边形
;; 运行：racket glsl/04-glsl-types/01-quad.rkt    点 X = 退出
;; =========================================================
...
;; 四边形 = 两个三角形 = 6 个顶点（角其实只有 4 个——这份"重复"是 06 课讲 EBO 的引子）。
...
;; 各出现两次——它们被两个三角形共用，正是这份"重复"引出 06 课的 EBO：
(define verts
...
(define-values (frame canvas)
  (make-window #:title "04-01 铺满窗口的四边形" #:width 400 #:height 400 #:draw draw))
