#lang racket/base
;; =========================================================
;; 18-model/05-draw.rkt —— 第五步：模型画出来了！（mesh → VAO + 光照）
;; 运行：racket glsl/18-model/05-draw.rkt    1/2 = cube / suzanne   点X = 退出
;; =========================================================
;; 前 4 步把 OBJ 文件变成了 CPU 侧的数组。本步把它们灌进 GPU 画出来——
;; 这一步**没有任何新 GL 函数**：VBO/EBO/VAO 是 02/06/08 课学过的，
;; 光照是 11 课学过的。新东西只是"把 obj-mesh 的数组接上去"。
;;
;; ★三步流水线的后两步（raylib 的对应关系）：
;;   ② 上传 rlLoadMesh → mesh->vao：verts→VBO、idx→EBO、配 attribute
;;   ③ 绘制 rlDrawMesh  → glDrawElements（索引绘制，06 课 EBO 同款）
;;
...
;; 顶点着色器：法线只受旋转影响 → 乘 mat3(uModel)（11 课同款）
(define vert-src
...
;; 片元着色器：固定方向光 + 漫反射 N·L（11 课同款，不贴图）
(define frag-src
  (glsl (version 330 core)
        (in vec3 vNormalW)
...
(define (draw)
  ...
  (define V (mat4-translate 0.0 0.0 -3.0))      ; 相机往后退 3 格
  (define M (mat4-rot-y (* t 40.0)))            ; 模型自转
...
(define-values (frame canvas)
  (make-window #:title "18-05 模型导入" #:width 800 #:height 600
