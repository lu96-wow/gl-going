#lang racket/base
;; =========================================================
;; 02-triangle/04-draw.rkt —— 第四步：画出来！三角形出现
;; 运行：racket glsl/02-triangle/04-draw.rkt    点 X = 退出
;; =========================================================
...
;; 每帧画什么：清屏 → 上程序 → 绑 VAO → 画。
;; prog/vao 定义在下面（draw 的 body 到显示后才被调用，前向引用合法）。
(define (draw)
  (glClearColor 0.10 0.12 0.20 1.0)
  (glClear GL_COLOR_BUFFER_BIT)
  (glUseProgram prog)        ; 把程序设为当前
  (glBindVertexArray vao)    ; 绑上 VAO（拿到"数据说明书"）
  ;; GL_TRIANGLES = 每 3 个顶点一组三角形；从第 0 个顶点画 3 个
  (glDrawArrays GL_TRIANGLES 0 3))

(define-values (frame canvas)
  (make-window #:title "02-04 第一个三角形" #:width 400 #:height 300 #:draw draw))

;; 初始化：程序 + VBO + VAO（同前两步，结果用普通 define 拿住）
