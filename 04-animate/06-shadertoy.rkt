#lang racket/base
;; =========================================================
;; 04-animate/06-shadertoy.rkt —— 第六步：综合，会动的着色玩具
;; 运行：racket 04-animate/06-shadertoy.rkt    点 X = 退出
;; =========================================================
...
;;   p = vUV*2-1                = 03 课算术：像素坐标，中心 (0,0)
;;   d = length(p)              = 03 课几何函数：到中心的距离 → 同心圆
;;   ring = fract(d*6 - uTime)  = 03 课 fract + 02 步减时间 → 环往外跑
;;   c = mix(深蓝,亮蓝,ring)     = 03 课混色：给环上色
...
(define-values (frame canvas)
  (make-window #:title "04-06 会动的着色玩具" #:width 400 #:height 400 #:draw draw))
