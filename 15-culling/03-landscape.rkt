#lang racket/base
;; =========================================================
;; 15-culling/03-landscape.rkt —— 第三步：可见性处理的方法谱系
;; 运行：racket 15-culling/03-landscape.rkt     R = 自转开/关   点X = 退出
;; =========================================================
;; 前两步学会了背面剔除。本步退一步看全景：**面剔除只是可见性处理的其中一种**。
;;
;; ★可见性处理的方法谱系——"谁该被画"在不同粒度上各有方法：
;;
;;   像素级   深度测试 z-buffer（07 课）：每个片元画之前比 z，远的画不过近的。
;;            管"前后遮挡"，最通用。
...
;;            最复杂（要判断"完全被挡"），但能省最多。
;;   老方法   画家算法（11 课透明）：按距离排序、从远到近画。不需要深度缓冲，
;;            但排序贵、且互相穿插时会出错。
;;
...
(define (draw)
  ...
  (glUniformMatrix4fv loc-mvp 1 #f
                      (mat4 (m4-mult (m4-mult P V) (m4-rot-y yaw))))
  (glBindVertexArray vao)
  (glDrawElements GL_TRIANGLES 6 GL_UNSIGNED_SHORT 0))

(define-values (frame canvas)
  (make-window #:title "15-03 可见性谱系（R 自转）"
