...
(define vao
  (send canvas with-gl-context
        (lambda ()
          ...
          ;; ★实例数组每行 7 float（偏移 xyz + 颜色 rgb + 相位 w），stride 用工具算 = 28
          (define s7 (glsl-stride-bytes 'vec3 'vec3 'float))
          (glVertexAttribPointer 1 (glsl-size 'vec3) GL_FLOAT #f s7 0)
          (glVertexAttribDivisor 1 1)
          (glEnableVertexAttribArray 1)
          (glVertexAttribPointer 2 (glsl-size 'vec3) GL_FLOAT #f s7 (glsl-stride-bytes 'vec3))
          (glVertexAttribDivisor 2 1)
          (glEnableVertexAttribArray 2)
          (glVertexAttribPointer 3 (glsl-size 'float) GL_FLOAT #f s7 (glsl-stride-bytes 'vec3 'vec3))
