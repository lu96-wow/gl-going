...
;; 接口（顶点输出 ↔ 片元输入）是否对得上。
;;
;; 这两步的 gl* 细节（glCreateShader、glShaderSource、状态检查、日志…）
...
;;   GL_VERTEX_SHADER   —— GL 常量，表示"这一段是顶点着色器"
;;   GL_FRAGMENT_SHADER —— GL 常量，表示"这一段是片元着色器"
;; 失败会自动抛错并带 GLSL/链接日志，不用自己查状态。
(send canvas with-gl-context
  (lambda ()
    (define prog (build-program (GL_VERTEX_SHADER vert-src)
