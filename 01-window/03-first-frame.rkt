...
(define gl-canvas%
  (class canvas%
    ...
    (define/override (on-paint)
      ...
      (with-gl-context
       (lambda ()
         ;; 画一帧 = 清屏。这两步是 OpenGL"状态机"风格的第一次亮相：
         ;;   GL 是一台状态机：先"记住"状态，再用状态执行。命令分两类——
         ;;   · 设状态：gl-clear-color 只"记住"清屏色，还没画
         ;;   · 执行：gl-clear 才"用"当前状态，把缓冲擦成那个色
         ;;   后面 02 课 gl-bind-buffer（设）→ gl-buffer-data（执行）还是这套：
         ;;   先绑、再操作，是 GL 的统一套路。
         (gl-clear-color 0.10 0.12 0.20 1.0)  ; ① 设状态：记住清屏色（深蓝灰 r g b a，各 0~1）
