...
;; 所以铺满窗口 = 显式给出 2 个三角形。
;;
;; 每个顶点 4 个 float：(位置.x, 位置.y, uv.x, uv.y)。
;; ★本课新构造器 vec4：和 02-03 讲的 vec2 同类（vec2/vec 的读法不再重复），只是
;;   "4 维"——(vec4 x y z w) = 一个 f32vector，四个 float。vec 仍是"打包成连续缓冲"。
;;   所以 (vec (vec4 ...) ...) = "6 个顶点，拼成一块连续的 24 个 float"。
;;   Racket 侧这个 vec4 是"造 f32vector 的构造器"，不是 shader 里的 vec4 类型——
;;   两个世界同名，都是"4 个 float 一组"。
